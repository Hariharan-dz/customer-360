...
export class NotificationCreatePageComponent implements OnInit {
  ...
  landingPageURL: string = '/notifications';
  ...
  constructor(private alertController: AlertController, public notifiService: NotificationUiService,
    ...
    this.isAbTesting = this.router.url.startsWith('/notifications/ab-testing');
    if (this.isAbTesting) {
      this.landingPageURL = '/notifications/ab-testing';
  ...
  loadTemplatePreviewByTemplateId() {
    ...
    if (template_id && !this.previewTemplateMap[template_id]) {
      this.notifiService.getTemplateById(template_id).subscribe((template: any) => {
  ...
  async getNotificationUserCount(segmentIdList: any, rule_type: string) {
    try {
      ...
      if (previousRuletype != rule_type || previousSegmentIdList != newSegmentIdList) {
        ...
        if (segmentIdList.length > 0) {
          ...
          const userSegment: any = await this.notifiService.getNotificationUserCount(segmentIdList.join(',')).toPromise();
          this.userCountDetails = await this.notifiService.getUserCountByRules({ rule_type, rules: userSegment.rules, channels: ['EMAIL', 'WEB_PUSH', 'MOBILE_PUSH', 'SMS', 'IN_APP_MESSAGE'] }).toPromise()
          if (--this.countApiTriggerCount == 0) {
            this.totalUserCount = this.userCountDetails.userCount;
          }
        }
      }
    } catch (err: any) {
