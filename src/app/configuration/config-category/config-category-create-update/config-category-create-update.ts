...
export class ConfigCategoryCreateUpdateComponent implements OnInit {
  ...
  validateForm() {
    this.form = new FormGroup({
      name: new FormControl(this.category.priority, [
        Validators.required,
  ...
  save() {
    ...
    if (this.rateLimit_type && this.rateLimit_value) this.category.rate_limit = this.rateLimit_value + ' PER ' + this.rateLimit_type;
    else this.category.rate_limit = null;
    this.notifiService.showLoader();
    if (this.isEditView) {
  ...
  //onCancel popup
  async onCancel() {
    this.notifiService.showCancelConfirmation(['/notifications/category']);
  }
