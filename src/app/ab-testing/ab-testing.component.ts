...
export class AbTestingComponent implements OnInit {
    ...
    buildData: (abTesing: any) => {
      return abTesing.map((abTesing: any) => {
        ...
        return {
          ...
          link: {
            id: abTesing.is_processed ? '/notifications/ab-testing/result/' + abTesing.id : '/notifications/ab-testing/' + abTesing.id
          },
          action: {
            view: '/notifications/ab-testing/result/' + abTesing.id,
            edit: '/notifications/ab-testing/' + abTesing.id,
            clone: '/notifications/ab-testing/clone/' + abTesing.id,
          }
        };
      });
    }
  ...
  defaultFilterCriteria: any = {
    ...
    status: ["CREATED", "TRIGGERED", "SCHEDULED"],
    category_id: []
  };

  @ViewChild('abTesting_grid') abTesting_grid: DataTableComponent | undefined;

  constructor(public notifiService: NotificationUiService, private datePipe: DatePipe, private alertController: AlertController) {
