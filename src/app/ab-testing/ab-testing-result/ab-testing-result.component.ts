...
export class AbTestingResultComponent implements OnInit {
  ...
  constructor(private actRouter: ActivatedRoute, public notifiService: NotificationUiService, private modalController: ModalController) { }
      ...
      try {
        ...
        if (this.notification && this.notification.variantList && summary) {
          ...
          this.notification.variantList.forEach((variant: any, index: number) => {
            variant.history_link = '/notifications/ab-testing/' + this.notification_id + '/history/' + variant.id;
            variant.summary = {
